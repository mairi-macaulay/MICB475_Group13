...
|---|---|---|---|
|1A-1: Edit metadata document to add column (sheetwashfreq_binned) that bins sheet washing frequency (sheetwashfreq_week) into high(0-2 wks)/medium(2-5 wks)/low(6+ wks) |1B-1: Edit metadata document to add column (last_shower_binned) that bins last shower day (lastshower_day) into recent (less than 1 day)/not recent(more than 1 day)|2-1: Import and demultiplex raw sequence reads|3-1: Edit metadata document to add column (hoursoutside_binned) that bins hours outside (hoursoutside) into high/low 
|1A-2: Import and demultiplex raw sequence reads (demux.qza)|1B-2: Import and demultiplex raw sequence reads|2-2: Perform sequence quality control, generate a feature table (table.qza) and representative sequences file (rep-seqs.qzv)|3-2: Import and demultiplex raw sequence reads|
|1A-3: Perform sequence quality control, generate a feature table (table.qza), and representative sequences file (rep-seqs.qzv)|1B-3: Perform sequence quality control, generate a feature table (table.qza), and representative sequences file (rep-seqs.qzv)|2-3: Filter metadata by gender|3-3: Perform sequence quality control, generate a feature table (table.qza), and representative sequences file (rep-seqs.qzv)|
