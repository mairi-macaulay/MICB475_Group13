# Core Microbiome Aim 1B (Sheet Washing Frequency and Sex, just female)
...
metadata <- sample_data(dorms_final_sheetwashfreq_deseq_female)
...
sample_data(dorms_final_sheetwashfreq_deseq_female) <- metadata
...
dorms_female_low <- subset_samples(dorms_female_RA, `new_sheetwash`=="low")
dorms_female_high <- subset_samples(dorms_female_RA, `new_sheetwash`=="high")
...
ggsave("heat_female_low.png", dorms_low_female_heat)
...
ggsave("heat_female_high.png", dorms_high_female_heat)


# Venn Diagram
# Setting detection and prevalence thresholds
low_ASVs <- core_members(dorms_female_low, detection=0.001, prevalence = 0.75)
high_ASVs <- core_members(dorms_female_high, detection=0.001, prevalence = 0.75)
...
ggsave("venn_dorms_low_high_female.png", dorms_low_high)
