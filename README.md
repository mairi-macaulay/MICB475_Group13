...
|---|---|---|---|
|1A-1: Edit metadata document to add column (sheetwashfreq_binned) that bins sheet washing frequency (sheetwashfreq_week) into high(0-2 wks)/medium(3-6 wks)/low(6+ wks) |1B-1: Edit metadata document to add column (last_shower_binned) that bins last shower day (lastshower_day) into recent (less than or equal 1 day)/not recent(more than 1 day)|2-1: Import and demultiplex raw sequence reads|3-1: Edit metadata document to add column (hoursoutside_binned) that bins hours outside (hoursoutside) into high/low 
