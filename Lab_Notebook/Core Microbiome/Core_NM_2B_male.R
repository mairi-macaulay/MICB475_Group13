...
dorms_male_low <- subset_samples(dorms_male_RA, `sheetwashfreq_binned`=="low")
dorms_male_high <- subset_samples(dorms_male_RA, `sheetwashfreq_binned`=="high")

metadata_of_male_high <- sample_data(dorms_male_high)
metadata_of_male_low <- sample_data(dorms_male_low)
...
dorms_low_male_heat
ggsave("heat_male_NM_low.png", dorms_low_male_heat)
...
ggsave("heat_male_NM_high.png", dorms_high_male_heat)


# Venn Diagram
# Setting detection and prevalence thresholds
low_male_ASVs <- core_members(dorms_male_low, detection=0.001, prevalence = 0.5)
high_male_ASVs <- core_members(dorms_male_high, detection=0.001, prevalence = 0.5)

print(low_ASVs)
print(high_ASVs)

dorms_list_low_high <- list(Low_Male_Frequency = low_male_ASVs, High_Male_Frequency = high_male_ASVs)
